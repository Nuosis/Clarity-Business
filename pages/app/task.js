...
import AllTask from '@/components/pages/app/task/AllTask'
import PriorityTask from '@/components/pages/app/task/PriorityTask'
import TodayTask from '@/components/pages/app/task/TodayTask'
import WeekTask from '@/components/pages/app/task/WeekTask'
import CreatedByMeTask from '@/components/pages/app/task/CreatedByMeTask'
...
export default function Task() {
    ...
    return (
        <Layout>
            <Tabs className='flex'>
                <div className={`bg-card-color min-w-[230px] w-[230px] p-4 xl:h-[calc(100svh-77px)] md:h-[calc(100svh-73px)] h-[calc(100svh-60px)] overflow-auto no-scrollbar lg:static fixed rounded-tl-2xl transition-all duration-300 ${taskSide ? 'z-[2] md:left-[13px] sm:left-2 left-0 shadow-shadow-lg' : '-left-full'}`}>
            ...
            {newTaskModal &&
                <>
                    ...
                    <div onClick={openNewTaskModal} className={`contents-[] fixed z-[5] w-full h-full left-0 top-0 bg-black-50 backdrop-blur-[5px]`}></div>
                </>
            }
        </Layout>
    )
